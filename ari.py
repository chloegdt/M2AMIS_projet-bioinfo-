...
def main():
            ...
            for data_type_2 in data_types:
                if data_type == data_type_2:  # Comparaison DBSCAN vs DBSCAN dans le même type
                    ...
                    if os.path.exists(file_dbscan1) and os.path.exists(file_dbscan2):
                        ...
                        ari_score = compute_ari(file_dbscan1, file_dbscan2)
                        if ari_score is not None:
                            results.append(f"DBSCAN {data_type} (Energy {energy}) vs DBSCAN {data_type} (Énergie {energy}): ARI = {ari_score:.4f}")
                        else:
            ...
            for data_type_2 in data_types:
                if data_type_1 != data_type_2:  # Comparaison entre types différents
                    ...
                    if os.path.exists(file_dbscan1) and os.path.exists(file_dbscan2):
                        ...
                        ari_score = compute_ari(file_dbscan1, file_dbscan2)
                        if ari_score is not None:
                            results.append(f"DBSCAN {data_type_1} (Énergie {energy}) vs DBSCAN {data_type_2} (Énergie {energy}): ARI = {ari_score:.4f}")
                        else:
    ...
    for energy in energies:
        for data_type_1 in data_types:
            ...
            if os.path.exists(file_dbscan) and os.path.exists(file_mcl):
                ...
                ari_score = compute_ari(file_dbscan, file_mcl)
                if ari_score is not None:
                    results.append(f"DBSCAN {data_type_1} (Énergie {energy}) vs MCL {data_type_1} (Énergie {energy}): ARI = {ari_score:.4f}")
                else:
            ...
            for data_type_2 in data_types:
                if data_type_1 != data_type_2:  # Comparaison DBSCAN (différents types) vs MCL (différents types)
                    ...
                    if os.path.exists(file_dbscan) and os.path.exists(file_mcl):
                        ...
                        ari_score = compute_ari(file_dbscan, file_mcl)
                        if ari_score is not None:
                            results.append(f"DBSCAN {data_type_1} (Énergie {energy}) vs MCL {data_type_2} (Énergie {energy}): ARI = {ari_score:.4f}")
                        else:
    ...
    with open("resultats_ari.txt", "w") as f:
        for line in results:
            f.write(line + "\n")

    print("Comparaisons terminées ! Résultats enregistrés dans 'resultats_ari.txt'.")

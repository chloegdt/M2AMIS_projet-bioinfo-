...
def create_dict_from_smiles(file_path):
    ...
    @return: A dictionary where:
        - Keys are lines starting with 'energy_'.
        - Values are lists of SMILES strings associated with each key.
    """
    smiles_dict = {}
    current_key = None
    ...
    with open(file_path, 'r') as file:
        for line in file:
            line = line.strip()
            if line.startswith("energy_"):
                current_key = line
                if current_key not in smiles_dict:
                    smiles_dict[current_key] = []
            else:
